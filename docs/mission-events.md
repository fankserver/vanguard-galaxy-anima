...
Missing dependencies prevent BepInEx startup. Disabled/unsupported mission capability prevents Anima hook installation. Hook-initialization failure rolls back installation. Observer failure stops the provider; capability loss is checked on Update and directly at save/quit writes. Stop unsubscribes events, removes hooks, disables registry writes and disposes the LLM client. Restart is required; there is no automatic fallback or catch-up.
