...
**Note:** C# records require `System.Runtime.CompilerServices.IsExternalInit`, which is missing from netstandard2.1. Add a polyfill shim at `VGAnima/Shims/IsExternalInit.cs` containing `namespace System.Runtime.CompilerServices; internal static class IsExternalInit { }` so records compile. This shim lives once for the whole project and is never referenced at runtime.

Also note: `SpaceStation` lives at `Source.Galaxy.POI.SpaceStation` (not under the `Station` sub-namespace). Types like `BarPatron`, `Bar`, `Salesman` live under `Source.Galaxy.POI.Station`/`Source.Galaxy.POI.Station.Patrons`. Files that use both need both `using` directives.
...
- Modify: `VGAnima/VGAnima.csproj` — add `<InternalsVisibleTo Include="VGAnima.Tests" />` so the test project can see internal types
- Modify: `VGAnima.Tests/VGAnima.Tests.csproj` — add a `<Reference Include="Assembly-CSharp">` (HintPath `..\VGAnima\lib\Assembly-CSharp.dll`) and a `UnityEngine.Modules` PackageReference, because `PatronContext` surfaces game types (`SpaceStation`, `Mission`) in its ctor and the test project must resolve them to JIT the constructor call
...
internal sealed class VanillaMissionSource : IMissionSource
{
    ...
    public Mission? Generate(MissionContext ctx)
    {
        ...
        // Instance GenerateMission signature (4 params, verified against stub):
        ...
        // defaults to Normal; vanilla MissionBoard uses MissionDifficultyExtension
        // to randomise across unlocked tiers, but the simple default is fine here.
        // targetLayer left null so the generator picks its own default.
        try
        {
...
**Note:** the `GenerateMission` instance signature was verified against the publicized stub (not just the IL dump). Arg order is `(poi, difficulty, rng, targetLayer)`. `MissionDifficulty` is a non-nullable enum — we hardcode `Normal` for v0.1. A follow-up could compute difficulty via `MissionDifficultyExtension.GetMissionDifficulty(...)` if level-scaled difficulty matters.
