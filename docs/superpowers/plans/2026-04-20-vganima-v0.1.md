...
**Note:** `DialogueManager` uses plain `Singleton<T>` (not `PersistentSingleton<T>` — VGTTS uses that for `SoundManager`). Verified against vanilla `Salesman.InteractWithPatron` IL which calls `Behaviour.Util.Singleton\`1<DialogueManager>::get_Instance`.
