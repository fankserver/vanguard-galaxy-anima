# Current compilation reference

The current compatibility target is Vanguard Galaxy **0.8.2.3**, original `Assembly-CSharp.dll` SHA-256 `a2aad60bc68c31baccd636587d3c5ba4e651eacda59b0af42cd4f17f864284fb`.

Run `make refresh-asm` with the owner-installed game and `assembly-publicizer`. This generates a stripped, publicized compile reference in ignored `.local-reference/`. Build/test validate the pinned source receipt and generated-reference checksum, then refresh the `lib/` symlink. If the installed original is present, its hash is checked too. A valid previously generated private reference can be reused without a game installation; it is still never published in the repository. Set `GAME_DIR` to your installation path when generating references or checking a local install. No game DLL or private receipt is distributed. A game update requires reinspection, not copying an old sibling stub. In an isolated worktree, set `VGMISSIONJOURNAL_DLL` to the current sibling Release DLL path.

Current personnel bindings:
- `Source.Personnel.CommanderSpecialization` replaces the old `Source.Crew` namespace.
- Named officers are `Source.Personnel.OfficerData` in `GamePlayer.officers`; vanilla still uses the `crewMembers` JSON key.
- `AbstractPersonnelData` provides first name, callsign and last name. The prompt retains its existing single-quoted callsign format and `Crew`/`RoleHint` wire fields.
- `Source.Personnel.CrewData` is aggregate ship crew, not the individual officer replacement.

System visits no longer bind a native travel method: the `TravelManager.JumpToSystem` prefix was replaced by the API's public travel events, so that identifier is no longer part of Anima's game-binding surface. Remaining Harmony targets are still validated against this pinned reference by the Cecil binding test.

The older decomp/wiki surveys are historical references, not current CLR binding specifications. In particular, old personnel and reward type names must be checked against the installed assembly before reuse. This change proves compilation and preserves formatting; it does not qualify all plugin hooks in-game. Host tests use a separate private runtime reference generated by `make refresh-test-asm` without stripping method bodies. It restores the managed constructors/getters required by five formerly failing tests, without changing their assertions or production code. The reference and checksums live in ignored `.local-test-reference/`; nothing is distributed. Existing verified test references can be reused offline. This does not make Unity-native APIs available to the host, and the existing 17 skipped cases remain skipped.
